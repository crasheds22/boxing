...
if ( $in{patientid} ) {
    # We are editing
    my $sql = "select b.patientid, a.username, a.accountname, DATE_FORMAT(b.dob, '%d/%m/%Y') as dob, b.condition, b.height, b.weight 
            from ACCOUNT a 
            join PATIENT b on a.accountid=b.patientid 
            where patientid=?";
    my $sth = $dbh->prepare( $sql );
    $sth->execute( $in{patientid} );
    ( $db{patientid}, $db{username}, $db{accountname}, $db{dob}, $db{condition}, $db{height}, $db{weight} ) = $sth->fetchrow_array;
    $sth->finish();
